...
char *lexer_get_line(Lexer *lexer, int line_num) {
    if (!lexer || !lexer->source || line_num <= 0) {
        return strdup("");
    }
    size_t idx = 0;
    int cur_line = 1;
...
static bool is_at_end(Lexer *lexer) { return lexer->current >= lexer->source_len; }
...
static bool matches_reserved_signed_special(Lexer *lexer, size_t index, const char *text) {
    ...
    return (!is_identifier_body_char(lexer->source[index + length])) != 0;
}
...
Token lexer_next_token(Lexer *lexer) {
    while (!is_at_end(lexer)) {
        ...
        if (c == '(') {
            advance(lexer);
            return make_token(lexer, TOKEN_LPAREN, "(", 1);
        }
        if (c == ')') {
            advance(lexer);
        ...
        if (c == '[') {
            advance(lexer);
            return make_token(lexer, TOKEN_LBRACKET, "[", 1);
        }
        if (c == ']') {
            advance(lexer);
            return make_token(lexer, TOKEN_RBRACKET, "]", 1);
        }
        if (c == '<') {
            advance(lexer);
            return make_token(lexer, TOKEN_LANGLE, "<", 1);
        }
        if (c == '>') {
            advance(lexer);
            return make_token(lexer, TOKEN_RANGLE, ">", 1);
        }
        if (c == ',') {
            advance(lexer);
        ...
        if (c == '=') {
            advance(lexer);
            return make_token(lexer, TOKEN_EQUALS, "=", 1);
        }
        if (c == ':') {
            advance(lexer);
        ...
        if (c == '@') {
            advance(lexer);
            return make_token(lexer, TOKEN_AT, "@", 1);
        }
        if (c == '~') {
            advance(lexer);
            return make_token(lexer, TOKEN_TILDE, "~", 1);
        }
        if (c == '#') {
            advance(lexer);
            return make_token(lexer, TOKEN_HASH, "#", 1);
        }
        if (c == '.') {
            advance(lexer);
        ...
        if (c == '-') {
            ...
            while (lookahead < lexer->source_len &&
                   (lexer->source[lookahead] == ' ' || lexer->source[lookahead] == '\t' ||
                    lexer->source[lookahead] == '\r')) {
                lookahead++;
            }

            if (lookahead + 1 < lexer->source_len && lexer->source[lookahead] == '0' &&
