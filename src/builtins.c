...
static bool writeback_ptr_range(Interpreter *interp, Expr **arg_nodes, Env *env, int start_index, int end_index,
                                Value result, const char *rule, int line, int col) {
    if (!arg_nodes) {
        return true;
    }
...
static const uint32_t cp1252_map[32] = {0x20AC, 0x0081, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021,
...
static char *dec_utf16_to_utf8(const unsigned char *buf, size_t sz, int little_endian) {
    ...
    while (i + 1 < sz) {
        uint16_t cu;
        if (little_endian) {
            cu = (uint16_t)(buf[i] | (buf[i + 1] << 8));
        } else {
            cu = (uint16_t)((buf[i] << 8) | buf[i + 1]);
        }
        i += 2;
        uint32_t cp;
        if (cu >= 0xD800 && cu <= 0xDBFF) {
            if (i + 1 < sz) {
                uint16_t cu2;
                if (little_endian) {
                    cu2 = (uint16_t)(buf[i] | (buf[i + 1] << 8));
                } else {
                    cu2 = (uint16_t)((buf[i] << 8) | buf[i + 1]);
                }
    ...
    if (i < sz) {
        ...
        out[op++] = (unsigned char)0xEF;
        out[op++] = (unsigned char)0xBF;
        out[op++] = (unsigned char)0xBD;
    }
    out[op] = '\0';
    return out;
}
...
static unsigned char *enc_utf8_to_utf16(const char *s, size_t *out_sz, int little_endian) {
    ...
    while (i < slen) {
        ...
        if (cp <= 0xFFFF) {
            ...
            if (op + 2 + 2 > outcap) {
                outcap *= 2;
                unsigned char *n = realloc(out, outcap);
                if (!n) {
                    free(out);
                    return NULL;
                }
            ...
            if (little_endian) {
                out[op++] = (unsigned char)(cu & 0xFF);
                out[op++] = (unsigned char)((cu >> 8) & 0xFF);
            } else {
                out[op++] = (unsigned char)((cu >> 8) & 0xFF);
                out[op++] = (unsigned char)(cu & 0xFF);
            }
            ...
            if (little_endian) {
                out[op++] = (unsigned char)(hi & 0xFF);
                out[op++] = (unsigned char)((hi >> 8) & 0xFF);
                out[op++] = (unsigned char)(lo & 0xFF);
                out[op++] = (unsigned char)((lo >> 8) & 0xFF);
            } else {
                out[op++] = (unsigned char)((hi >> 8) & 0xFF);
                out[op++] = (unsigned char)(hi & 0xFF);
                out[op++] = (unsigned char)((lo >> 8) & 0xFF);
...
static size_t utf8_codepoint_count(const char *s) {
    ...
    while (*p) {
        ...
        if (c < 0x80) {
            ...
            if (p[1] != '\0' && p[2] != '\0' && p[3] != '\0' && (p[1] & 0xC0) == 0x80 && (p[2] & 0xC0) == 0x80 &&
                (p[3] & 0xC0) == 0x80) {
                codepoint = ((uint32_t)(c & 0x07) << 18) | ((uint32_t)(p[1] & 0x3F) << 12) |
                            ((uint32_t)(p[2] & 0x3F) << 6) | (uint32_t)(p[3] & 0x3F);
                seq_len = 4;
                if (codepoint < 0x10000 || codepoint > 0x10FFFF) {
                    seq_len = 0; /* overlong or out of range */
                }
            }
        }
...
static char *int_to_base_prefixed_str(int64_t val, int base) {
    ...
    memcpy(out + w, pref, pref_len);
    w += pref_len;
    memcpy(out + w, digs, digs_len);
    w += digs_len;
    out[w] = '\0';
    return out;
}
...
static char *float_to_base_prefixed_str(double val, int base, int base_is_nan) {
    ...
    while (fpos > 0 && frac_digits[fpos - 1] == '0') {
        fpos--;
    }
    frac_digits[fpos] = '\0';
...
static int parse_prefixed_int_string(const char *text, int64_t *out_val, int *out_base) {
    ...
    if (!parse_numeric_prefix(s, &base, &digits) || !digits || !*digits) {
        return 0;
    }
    if (strchr(digits, '.')) {
        return 0;
...
static int parse_prefixed_flt_string(const char *text, double *out_val, int *out_base, int *out_base_is_nan) {
    if (!text || !out_val || !out_base || !out_base_is_nan) {
        return 0;
    }
        ...
        if (c == '\\') {
            ...
            case 'u': {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (v < 0) {
                        p->error = "Invalid unicode escape";
                        jb_free(&sb);
                        return NULL;
                    }
                    code = (code << 4) | v;
...
static JsonValue *json_parse(const char *text, const char **err) {
    ...
    if (!v || p.error) {
        if (err) {
            *err = p.error ? p.error : "Invalid JSON";
        }
        return NULL;
    }
    json_skip_ws(&p);
...
static const char *ser_env_id(SerCtx *ctx, Env *env, int *state) {
    ...
    if (ctx->env_count + 1 > ctx->env_cap) {
        ...
        if (!ctx->envs || !ctx->env_ids || !ctx->env_state) {
            fprintf(stderr, "Out of memory\n");
            exit(1);
        }
        ctx->env_cap = new_cap;
...
static const char *ser_func_id(SerCtx *ctx, Func *func, int *state) {
    ...
    if (ctx->func_count + 1 > ctx->func_cap) {
        ...
        if (!ctx->funcs || !ctx->func_ids || !ctx->func_state) {
            fprintf(stderr, "Out of memory\n");
            exit(1);
        }
...
static void ser_expr(JsonBuf *jb, SerCtx *ctx, Interpreter *interp, Expr *expr) {
    ...
    case EXPR_FLOAT: {
        ...
        if (isnan(expr->as.flt_value.value)) {
            ...
            if (signbit(expr->as.flt_value.value)) {
                jb_append_json_string(jb, "-INF");
            } else {
                jb_append_json_string(jb, "INF");
            }
        } else {
    ...
    case EXPR_INDEX: {
        ...
        jb_append_str(jb, (int)expr->as.index.is_map ? "true" : "false");
        jb_append_char(jb, '}');
        return;
    }
    case EXPR_RANGE: {
...
static void ser_stmt(JsonBuf *jb, SerCtx *ctx, Interpreter *interp, Stmt *stmt) {
    ...
    case STMT_FUNC: {
        ...
        for (size_t i = 0; i < stmt->as.func_stmt.params.count; i++) {
            ...
            jb_append_str(jb, (int)p->coerced ? "true" : "false");
            json_obj_field(jb, &pf, "name");
            jb_append_json_string(jb, p->name ? p->name : "");
            json_obj_field(jb, &pf, "default");
            if (p->default_value) {
                ser_expr(jb, ctx, interp, p->default_value);
            } else {
                jb_append_str(jb, "null");
            }
            jb_append_char(jb, '}');
    ...
    case STMT_TRY: {
        ...
        if (stmt->as.try_stmt.catch_name) {
            jb_append_json_string(jb, stmt->as.try_stmt.catch_name);
        } else {
            jb_append_str(jb, "null");
        }
...
static void ser_value(JsonBuf *jb, SerCtx *ctx, Interpreter *interp, Value v) {
    ...
    case VAL_THREAD: {
        ...
        json_obj_field(jb, &first, "block");
        if (th->body) {
            ser_stmt(jb, ctx, interp, th->body);
        } else {
            jb_append_str(jb, "null");
        }
        jb_append_char(jb, '}');
...
static Env *unser_env_get(UnserCtx *ctx, const char *id) {
    for (size_t i = 0; i < ctx->envs.count; i++) {
        if (strcmp(ctx->envs.ids[i], id) == 0) {
            return ctx->envs.envs[i];
        }
    }
    return NULL;
}
...
static Func *unser_func_get(UnserCtx *ctx, const char *id) {
    for (size_t i = 0; i < ctx->funcs.count; i++) {
        if (strcmp(ctx->funcs.ids[i], id) == 0) {
            return ctx->funcs.funcs[i];
        }
    }
    return NULL;
}
...
static void unser_func_set(UnserCtx *ctx, const char *id, Func *func) {
    if (ctx->funcs.count + 1 > ctx->funcs.cap) {
        ...
        if (!ctx->funcs.ids || !ctx->funcs.funcs) {
            fprintf(stderr, "Out of memory\n");
            exit(1);
        }
...
static Expr *deser_default_expr(JsonValue *raw, UnserCtx *ctx, Interpreter *interp, const char **err) {
    ...
    if (v.type == VAL_BOOL) {
        return expr_bool(v.as.boolean, 1, 1);
    }
    if (v.type == VAL_INT) {
...
static Expr *deser_expr(JsonValue *obj, UnserCtx *ctx, Interpreter *interp, const char **err) {
    ...
    if (strcmp(name, "Literal") == 0) {
        ...
        if (strcmp(lt, "bool") == 0) {
            ...
            if (val->type == JSON_BOOL) {
                return expr_bool(val->as.boolean != 0, line, col);
            }
            if (val->type == JSON_STR) {
                if (strcmp(val->as.str, "TRUE") == 0 || strcmp(val->as.str, "true") == 0) {
                    return expr_bool(true, line, col);
                }
                if (strcmp(val->as.str, "FALSE") == 0 || strcmp(val->as.str, "false") == 0) {
        ...
        if (strcmp(lt, "float") == 0) {
            ...
            if (val && val->type == JSON_NUM) {
                f = val->as.num;
            } else if (val && val->type == JSON_STR) {
                f = strtod(val->as.str, NULL);
            }
            JsonValue *basev = json_obj_get(obj, "base");
    ...
    if (strcmp(name, "TensorLiteral") == 0) {
        ...
        if (items && items->type == JSON_ARR) {
            for (size_t i = 0; i < items->as.arr.count; i++) {
                ...
                if (it) {
                    expr_list_add(&t->as.tns_items, it);
                }
            }
        }
        return t;
    ...
    if (strcmp(name, "IndexExpression") == 0) {
        ...
        bool is_map = false;
        if (is_map_v && is_map_v->type == JSON_BOOL) {
            is_map = ((is_map_v->as.boolean ? true : false) != 0);
        }
        ...
        if (indices && indices->type == JSON_ARR) {
            for (size_t i = 0; i < indices->as.arr.count; i++) {
                ...
                if (it) {
                    expr_list_add(&idx->as.index.indices, it);
                }
            }
        }
        return idx;
            ...
            for (size_t i = 0; i < values->as.obj.count; i++) {
                ...
                if (vv && vv->type == JSON_OBJ) {
                    ...
                    if (vt && vt->type == JSON_STR && strcmp(vt->as.str, "PTR") == 0) {
                        ...
                        if (target) {
                            entry->alias_target = strdup(target);
                        }
                        entry->decl_type =
                            decl_type_from_name((vtype && vtype->type == JSON_STR) ? vtype->as.str : NULL);
...
static Value deser_val(JsonValue *obj, UnserCtx *ctx, Interpreter *interp, const char **err) {
    ...
    if (strcmp(tp, "bool") == 0) {
        ...
        if (v->type == JSON_STR) {
            if (strcmp(v->as.str, "TRUE") == 0 || strcmp(v->as.str, "true") == 0) {
                return value_bool(true);
            }
            if (strcmp(v->as.str, "FALSE") == 0 || strcmp(v->as.str, "false") == 0) {
    ...
    if (strcmp(tp, "tensor") == 0) {
        ...
        for (size_t i = 0; i < ndim; i++) {
            shp[i] = (size_t)shape->as.arr.items[i]->as.num;
        }
...
static Value builtin_shape(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    for (size_t i = 0; i < ndim; i++) {
        items[i] = value_int((int64_t)t->shape[i]);
    }
    size_t out_shape[1];
    out_shape[0] = ndim;
                        ...
                        for (size_t kx = 0; kx < kw; kx++) {
                            for (size_t ky = 0; ky < kh; ky++) {
                                for (size_t ic = 0; ic < in_c; ic++) {
                                    ...
                                    if (in_x < 0 || in_y < 0 || (size_t)in_x >= in_w || (size_t)in_y >= in_h) {
                                        continue; // zero pad
                                    }
                                    size_t in_off = ((size_t)in_x * x->strides[0]) + ((size_t)in_y * x->strides[1]) +
                                                    (ic * x->strides[2]);
                                    size_t k_off = (kx * k->strides[0]) + (ky * k->strides[1]) + (ic * k->strides[2]) +
                                                   (oc * k->strides[3]);
        ...
        if (out_decl == TYPE_INT) {
            ...
            for (size_t kpos = 0; kpos < k->length; kpos++) {
                ...
                for (size_t d = 0; d < x->ndim; d++) {
                    ...
                    if ((size_t)rel >= x->shape[d]) {
                        rel = (int64_t)x->shape[d] - 1;
                    }
...
static Value builtin_append(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (out_elem_type == TYPE_UNKNOWN) {
        if (old_len == 0) {
            out_elem_type = appended_decl;
        }
    } else {
        if (appended_decl != out_elem_type) {
            out_elem_type = TYPE_UNKNOWN;
        }
    }
...
static Value builtin_scat(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    for (size_t d = 0; d < rank; d++) {
        ...
        if (l < 0) {
            l = (int64_t)dst->shape[d] + l + 1;
        }
        if (h < 0) {
            h = (int64_t)dst->shape[d] + h + 1;
        }
...
static Value builtin_lcm(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (args[0].type == VAL_INT) {
        ...
        int64_t b = args[1].as.i;
        if (a == 0 || b == 0) {
            return value_int_base(0, out_base);
        }
...
static int value_deep_eq_impl(Value a, Value b, EqSeenSet *seen) {
    ...
    case VAL_TENSOR: {
        ...
        for (size_t i = 0; i < ta->length; i++) {
            if (!value_deep_eq_impl(ta->data[i], tb->data[i], seen)) {
                return 0;
            }
        }
        return 1;
    ...
    case VAL_MAP: {
        ...
        if (eq_seen_contains(seen, ma, mb, VAL_MAP)) {
            return 1;
        }
        ...
        for (size_t i = 0; i < ma->count; i++) {
            ...
            for (size_t j = 0; j < mb->count; j++) {
                if (value_deep_eq_impl(ma->items[i].key, mb->items[j].key, seen)) {
                    if (!value_deep_eq_impl(ma->items[i].value, mb->items[j].value, seen)) {
                        return 0;
                    }
                    found = 1;
                    break;
...
static Value builtin_and(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    (void)line;
    (void)col;
    return value_bool((value_truthiness(args[0]) && value_truthiness(args[1])) != 0);
}
...
static Value builtin_not(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    (void)line;
    (void)col;
    return value_bool((!value_truthiness(args[0])) != 0);
}
...
static Value builtin_int(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (args[0].type == VAL_BOOL) {
        return value_int_base((int)args[0].as.boolean ? 1 : 0, 2);
    }
    if (args[0].type == VAL_INT) {
...
static Value builtin_join(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (first_type == VAL_STR) {
        ...
        bool separator_mode = (first_len == 1 && argc >= 3) != 0;
        if (separator_mode) {
...
static Value builtin_in(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    for (size_t i = 0; i < t->length; i++) {
        if (value_deep_eq(args[0], t->data[i])) {
            return value_bool(true);
        }
    }
    return value_bool(false);
...
static Value builtin_slice(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (args[0].type == VAL_STR) {
        ...
        int64_t end = args[2].as.i;
        if (start < 0) {
            start = (int64_t)len + start + 1;
        }
        if (end < 0) {
            end = (int64_t)len + end + 1;
        }
...
static Value builtin_warn(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (out_len + piece_len + 1 > cap) {
            size_t newcap = cap;
            while (out_len + piece_len + 1 > newcap) {
                newcap *= 2;
            }
...
static Value builtin_readfile(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
    ...
    if (clen >= sizeof(codelb)) {
        clen = sizeof(codelb) - 1;
    }
    for (size_t i = 0; i < clen; i++) {
        codelb[i] = (char)tolower((unsigned char)coding[i]);
    }
    codelb[clen] = '\0';
    ...
    if (strcmp(codelb, "utf-8") == 0 || strcmp(codelb, "utf8") == 0 || strcmp(codelb, "utf-8-bom") == 0 ||
        strcmp(codelb, "utf-8 bom") == 0) {
        size_t start = 0;
    ...
    if (strstr(codelb, "utf-16") != NULL) {
        ...
        if (strstr(codelb, "be") != NULL) {
            little = 0;
        }
        if (strstr(codelb, "le") != NULL) {
            little = 1;
        }
...
static Value builtin_writefile(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
    ...
    if (strcmp(codelb, "utf-8-bom") == 0 || strcmp(codelb, "utf-8 bom") == 0) {
        ...
        if (towrite > 0) {
            if (fwrite(blob, 1, towrite, f) != towrite) {
                fclose(f);
                return value_bool(false);
            }
        }
    ...
    if (args[0].type == VAL_TENSOR) {
        ...
        for (int j = 0; j < argc && !seeded; j++) {
            ...
            for (size_t i = 0; i < tj->length; i++) {
                ...
                if (etype == TYPE_STR && v.type == VAL_STR) {
...
static Value builtin_valuein(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
    ...
    for (size_t i = 0; i < m->count; i++) {
        if (value_deep_eq(args[0], m->items[i].value)) {
            return value_bool(true);
        }
    }
    return value_bool(false);
...
static Value builtin_assign(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (target->type == EXPR_INDEX) {
        ...
        if (ns_buffer_active() && !ns_buffer_is_prepare_thread()) {
            ...
            int buffered_line = 0;
            int buffered_col = 0;
            if (!ns_buffer_assign_index(interp, env, target, rhs, line, col, &buffered_error, &buffered_line,
                                        &buffered_col)) {
...
static Value builtin_ilen(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (v == 0) {
        return value_int_base(1, out_base);
    }
...
static Value builtin_main(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (call_src->value.type == VAL_STR && primary_src->value.type == VAL_STR && call_src->value.as.s &&
        primary_src->value.as.s) {
...
static Value builtin_extend(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (loaded_name && loaded_name[0] != '\0') {
        ...
        const char *existing_namespaces = (namespaces_entry && namespaces_entry->initialized &&
                                           namespaces_entry->value.type == VAL_STR && namespaces_entry->value.as.s)
...
static int module_export_bindings(Interpreter *interp, Env *caller_env, Env *mod_env, const char *alias, int line,
    ...
    if (namespaces_entry && namespaces_entry->initialized && namespaces_entry->value.type == VAL_STR &&
        namespaces_entry->value.as.s && namespaces_entry->value.as.s[0] != '\0') {
...
static Value builtin_tensor(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (argc == 2) {
        ...
        for (size_t i = 0; i < ndim; i++) {
            ...
            if (v.as.i <= 0) {
                free(shape);
                RUNTIME_ERROR(interp, "Shape lengths must be positive", line, col);
            }
            shape[i] = (size_t)v.as.i;
            // check overflow
            if (total > SIZE_MAX / shape[i]) {
                free(shape);
                RUNTIME_ERROR(interp, "Shape too large", line, col);
            }
            total *= shape[i];
        }
...
static int parallel_worker(void *arg) {
    ...
    for (size_t i = 0; i < ps->func->params.count; i++) {
        ...
        if (param->default_value) {
            ...
            if (thr_interp->error) {
                ...
                if (ps->err_lines) {
                    ps->err_lines[ps->index] = thr_interp->error_line;
                }
                if (ps->err_cols) {
                    ps->err_cols[ps->index] = thr_interp->error_col;
                }
            ...
            if (ps->err_lines) {
                ps->err_lines[ps->index] = 0;
            }
            if (ps->err_cols) {
                ps->err_cols[ps->index] = 0;
            }
            env_free(call_env);
            free(thr_interp);
            free(ps);
    ...
    if (res.status == EXEC_ERROR && res.error) {
        ...
        if (ps->err_lines) {
            ps->err_lines[ps->index] = res.error_line;
        }
        if (ps->err_cols) {
            ps->err_cols[ps->index] = res.error_column;
        }
    } else {
...
static Value builtin_parallel(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    ...
    for (size_t i = 0; i < n; i++) {
        if (errors[i]) {
            first_err = errors[i];
            first_line = err_lines[i];
            first_col = err_cols[i];
            break;
        }
    }
