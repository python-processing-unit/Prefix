...
static GuiWindow *gui_find_window_by_id(int id) {
    for (size_t i = 0; i < g_gui.count; i++) {
        if (g_gui.items[i].id == id) {
            return &g_gui.items[i];
        }
    }
    return NULL;
}
...
static void gui_draw_window(HDC hdc, GuiWindow *win, int dst_w, int dst_h) {
    ...
    StretchDIBits(hdc, 0, 0, out_w, out_h, 0, 0, src_w, src_h, bgra, &bmi, DIB_RGB_COLORS, SRCCOPY);
...
static int extract_image_rgba(Interpreter *interp, Value v, uint8_t **out_rgba, int *out_w, int *out_h, int line,
                              int col) {
...
static int parse_window_kind(Interpreter *interp, const char *kind, int *out_scale_default, DWORD *inout_style,
                             DWORD *inout_exstyle, int *fullscreen, int line, int col) {
    if (!kind || !out_scale_default || !inout_style || !inout_exstyle || !fullscreen) {
        return 0;
    }
...
static Value op_create_window(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    gui_pump_messages();
    ...
    if (!wtitle) {
        return fail(interp, "GUI_CREATE_WINDOW failed: invalid title encoding", line, col);
    }
    ...
    HWND hwnd = CreateWindowExW(exstyle, k_gui_class_name, wtitle, style, CW_USEDEFAULT, CW_USEDEFAULT, win_w, win_h,
                                NULL, NULL, g_gui.hinstance, NULL);
    free(wtitle);

    if (!hwnd) {
        return fail(interp, "GUI_CREATE_WINDOW failed: CreateWindowExW failed", line, col);
    }
...
static Value op_show_image(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (argc < 2) {
        return fail(interp, "GUI_SHOW_IMAGE expects 2 arguments", line, col);
    }
...
static Value op_close_window(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                             int col) {
    (void)arg_nodes;
    (void)env;
    gui_pump_messages();
    ...
    if (argc < 1) {
        return fail(interp, "GUI_CLOSE_WINDOW expects 1 argument", line, col);
    }
    int ok = 1;
...
static Value op_minimize(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    gui_pump_messages();
    if (argc < 1) {
        return fail(interp, "GUI_MINIMIZE expects 1 argument", line, col);
    }
    int ok = 1;
    ...
    GuiWindow *win = gui_find_window_by_id(wid);
    if (!win || !IsWindow(win->hwnd)) {
        return fail(interp, "GUI_MINIMIZE: invalid window handle", line, col);
    }
...
static Value op_maximize(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    gui_pump_messages();
    if (argc < 1) {
        return fail(interp, "GUI_MAXIMIZE expects 1 argument", line, col);
    }
    int ok = 1;
    ...
    GuiWindow *win = gui_find_window_by_id(wid);
    if (!win || !IsWindow(win->hwnd)) {
        return fail(interp, "GUI_MAXIMIZE: invalid window handle", line, col);
    }
...
static Value op_to_front(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    gui_pump_messages();
    if (argc < 1) {
        return fail(interp, "GUI_TO_FRONT expects 1 argument", line, col);
    }
    int ok = 1;
    ...
    GuiWindow *win = gui_find_window_by_id(wid);
    if (!win || !IsWindow(win->hwnd)) {
        return fail(interp, "GUI_TO_FRONT: invalid window handle", line, col);
    }
...
static Value op_to_back(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    gui_pump_messages();
    if (argc < 1) {
        return fail(interp, "GUI_TO_BACK expects 1 argument", line, col);
    }
    int ok = 1;
    ...
    GuiWindow *win = gui_find_window_by_id(wid);
    if (!win || !IsWindow(win->hwnd)) {
        return fail(interp, "GUI_TO_BACK: invalid window handle", line, col);
    }
...
static Value op_window(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    gui_pump_messages();
    if (argc < 1) {
        return fail(interp, "WINDOW expects 1 argument", line, col);
    }
    int ok = 1;
    ...
    GuiWindow *win = gui_find_window_by_id(wid);
    if (!win || !IsWindow(win->hwnd)) {
        return fail(interp, "WINDOW: invalid window handle", line, col);
    }

    RECT rc;
    if (!GetWindowRect(win->hwnd, &rc)) {
        return fail(interp, "WINDOW: failed to query window size", line, col);
    }
    int w = rc.right - rc.left;
    int h = rc.bottom - rc.top;
    if (w < 1) {
...
static Value op_screenshot(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (!hdc_screen) {
        return fail(interp, "SCREENSHOT failed: GetDC failed", line, col);
    }
