...
static size_t pixel_offset(const Tensor *t, size_t x, size_t y) { return (x * t->strides[0]) + (y * t->strides[1]); }
...
static Value copy_image_checked(Interpreter *interp, Value src, const char *opname, int line, int col) {
    ImageView iv;
    if (!image_from_value(interp, src, opname, line, col, &iv)) {
        return value_null();
    }
...
static void put_pixel_rgba(Tensor *t, int x, int y, const int rgba[4], int mix_alpha) {
    ...
    if ((size_t)x >= t->shape[0] || (size_t)y >= t->shape[1]) {
        return;
    }
...
static void fill_polygon(Tensor *t, const int *pts, size_t npts, const int rgba[4]) {
    ...
    if (maxy >= (int)t->shape[1]) {
        maxy = (int)t->shape[1] - 1;
    }
...
static void draw_ellipse(Tensor *t, int cx, int cy, int rx, int ry, const int rgba[4], int fill, int thickness) {
    ...
    if (fill) {
        for (int y = -ry; y <= ry; y++) {
            ...
            for (int x = -dx; x <= dx; x++) {
                put_pixel_rgba(t, cx + x, cy + y, rgba, 1);
            }
        }
    }
...
static int encoder_clsid_for_mime(const WCHAR *mime, CLSID *out_clsid) {
    ...
    if (pGdipGetImageEncodersSize(&n, &sz) != 0 || n == 0 || sz == 0) {
        return 0;
    }
...
static Value load_with_gdiplus(Interpreter *interp, const char *path, int line, int col) {
    if (!ensure_gdiplus(interp, line, col)) {
        return value_null();
    }
    WCHAR *wpath = NULL;
    if (!utf8_to_wide(path, &wpath)) {
        return fail(interp, "image: invalid UTF-8 path", line, col);
    }
...
static Value save_with_gdiplus(Interpreter *interp, Value imgv, const char *path, const WCHAR *mime, int quality,
                               int line, int col) {
    ImageView iv;
    if (!image_from_value(interp, imgv, "SAVE_*", line, col, &iv)) {
        return value_null();
    }
    if (!ensure_gdiplus(interp, line, col)) {
        return value_null();
    ...
    if (!bgra) {
        set_runtime_error(interp, "image: out of memory", line, col);
        return value_null();
    }
    ...
    if (st != 0) {
        set_runtime_error(interp, "image: failed to save image", line, col);
        return value_null();
    }
...
static Value op_load_png(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 1, 1, "LOAD_PNG", line, col)) {
        return value_null();
    }
...
static Value op_load_jpeg(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 1, 1, "LOAD_JPEG", line, col)) {
        return value_null();
    }
...
static Value op_load_bmp(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 1, 1, "LOAD_BMP", line, col)) {
        return value_null();
    }
...
static Value op_save_png(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 3, "SAVE_PNG", line, col)) {
        return value_null();
    }
    ...
    int quality = 100;
    if (argc >= 3) {
        quality = (int)expect_int(interp, args[2], "SAVE_PNG", line, col);
    }
    if (interp->error) {
        return value_null();
    }
...
static Value op_save_jpeg(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 3, "SAVE_JPEG", line, col)) {
        return value_null();
    }
    ...
    int quality = 85;
    if (argc >= 3) {
        quality = (int)expect_int(interp, args[2], "SAVE_JPEG", line, col);
    }
    if (interp->error) {
        return value_null();
    }
...
static Value op_save_bmp(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 2, "SAVE_BMP", line, col)) {
        return value_null();
    }
...
static Value op_polygon(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 3, 5, "POLYGON", line, col)) {
        return value_null();
    }
    ...
    int thickness = 1;
    if (argc >= 4) {
        fill = (int)expect_int(interp, args[3], "POLYGON", line, col);
    }
    if (argc >= 5) {
        thickness = (int)expect_int(interp, args[4], "POLYGON", line, col);
    }
    if (interp->error) {
        free(pts);
        value_free(out);
...
static Value op_ellipse(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 5, 7, "ELLIPSE", line, col)) {
        return value_null();
    }
    ...
    int thickness = 1;
    if (argc >= 6) {
        fill = (int)expect_int(interp, args[5], "ELLIPSE", line, col);
    }
    if (argc >= 7) {
        thickness = (int)expect_int(interp, args[6], "ELLIPSE", line, col);
    }
    if (interp->error) {
        value_free(out);
        return value_null();
...
static Value threshold_channel(Interpreter *interp, Value imgv, Value thv, Value colorv, int ch, const char *opname,
                               int line, int col) {
    ...
    for (size_t x = 0; x < t->shape[0]; x++) {
        for (size_t y = 0; y < t->shape[1]; y++) {
            ...
            if (v <= th) {
                for (size_t c = 0; c < 4; c++) {
                    t->data[off + c] = value_int((int64_t)color[c]);
                }
            }
        }
    }
...
static Value op_threshhold_a(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                             int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 3, "THRESHHOLD_A", line, col)) {
        return value_null();
    }
    return threshold_channel(interp, args[0], args[1], (argc >= 3) ? args[2] : value_null(), 3, "THRESHHOLD_A", line,
                             col);
}

static Value op_threshhold_r(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                             int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 3, "THRESHHOLD_R", line, col)) {
        return value_null();
    }
    return threshold_channel(interp, args[0], args[1], (argc >= 3) ? args[2] : value_null(), 0, "THRESHHOLD_R", line,
                             col);
}

static Value op_threshhold_g(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                             int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 3, "THRESHHOLD_G", line, col)) {
        return value_null();
    }
    return threshold_channel(interp, args[0], args[1], (argc >= 3) ? args[2] : value_null(), 1, "THRESHHOLD_G", line,
                             col);
}

static Value op_threshhold_b(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                             int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 3, "THRESHHOLD_B", line, col)) {
        return value_null();
    }
    return threshold_channel(interp, args[0], args[1], (argc >= 3) ? args[2] : value_null(), 2, "THRESHHOLD_B", line,
                             col);
}

static Value resize_impl(Interpreter *interp, Value imgv, int new_w, int new_h, int antialiasing, const char *opname,
                         int line, int col) {
    ImageView iv;
    if (!image_from_value(interp, imgv, opname, line, col, &iv)) {
        return value_null();
    }
    if (new_w <= 0 || new_h <= 0) {
        return fail(interp, "new dimensions must be > 0", line, col);
    }
    ...
    for (int x = 0; x < new_w; x++) {
        for (int y = 0; y < new_h; y++) {
            ...
            if (!antialiasing) {
                ...
                for (size_t c = 0; c < 4; c++) {
                    dt->data[doff + c] = value_int(st->data[soff + c].as.i);
                }
            } else {
...
static Value op_scale(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 3, 4, "SCALE", line, col)) {
        return value_null();
    }
    ImageView iv;
    if (!image_from_value(interp, args[0], "SCALE", line, col, &iv)) {
        return value_null();
    }
    ...
    int aa = 1;
    if (argc >= 4) {
        aa = (int)expect_int(interp, args[3], "SCALE", line, col);
    }
    if (interp->error) {
        return value_null();
    }
    if (sx <= 0.0 || sy <= 0.0) {
        return fail(interp, "SCALE factors must be > 0", line, col);
    }
...
static Value op_resize(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 3, 4, "RESIZE", line, col)) {
        return value_null();
    }
    ...
    int aa = 1;
    if (argc >= 4) {
        aa = (int)expect_int(interp, args[3], "RESIZE", line, col);
    }
    if (interp->error) {
        return value_null();
    }
...
static Value op_rotate(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 2, "ROTATE", line, col)) {
        return value_null();
    }
    ImageView iv;
    if (!image_from_value(interp, args[0], "ROTATE", line, col, &iv)) {
        return value_null();
    }
...
static Value op_blit(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 4, 5, "BLIT", line, col)) {
        return value_null();
    }
    ImageView src;
    ImageView dst;
    if (!image_from_value(interp, args[0], "BLIT", line, col, &src)) {
        return value_null();
    }
    if (!image_from_value(interp, args[1], "BLIT", line, col, &dst)) {
        return value_null();
    }
    ...
    int mix = 1;
    if (argc >= 5) {
        mix = (int)expect_int(interp, args[4], "BLIT", line, col);
    }
    if (interp->error) {
        return value_null();
    }
    ...
    for (size_t sx = 0; sx < src.w; sx++) {
        for (size_t sy = 0; sy < src.h; sy++) {
            int dx = (int)sx + ox;
            int dy = (int)sy + oy;
            if (dx < 0 || dy < 0 || (size_t)dx >= dst.w || (size_t)dy >= dst.h) {
                continue;
            }
            size_t soff = pixel_offset(src.t, sx, sy);
            int rgba[4] = {(int)src.t->data[soff + 0].as.i, (int)src.t->data[soff + 1].as.i,
                           (int)src.t->data[soff + 2].as.i, (int)src.t->data[soff + 3].as.i};
...
static Value op_grayscale(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 1, 1, "GRAYSCALE", line, col)) {
        return value_null();
    }
...
static Value op_replace_color(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 3, 3, "REPLACE_COLOR", line, col)) {
        return value_null();
    }
    int target[4];
    int repl[4];
    if (!parse_color_rgba(interp, args[1], target, "REPLACE_COLOR", line, col)) {
        return value_null();
    }
    if (!parse_color_rgba(interp, args[2], repl, "REPLACE_COLOR", line, col)) {
        return value_null();
    }
    ...
    for (size_t x = 0; x < t->shape[0]; x++) {
        for (size_t y = 0; y < t->shape[1]; y++) {
            ...
            if (same) {
                for (size_t c = 0; c < 4; c++) {
                    t->data[off + c] = value_int((int64_t)repl[c]);
                }
            }
        }
    }
...
static Value op_blur(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 2, "BLUR", line, col)) {
        return value_null();
    }
    ...
    if (radius < 0) {
        return fail(interp, "BLUR radius must be >= 0", line, col);
    }
    if (radius == 0) {
        return copy_image_checked(interp, args[0], "BLUR", line, col);
    }

    ImageView iv;
    if (!image_from_value(interp, args[0], "BLUR", line, col, &iv)) {
        return value_null();
    }
...
static Value op_edge(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 1, 1, "EDGE", line, col)) {
        return value_null();
    }
    ...
    ImageView i1;
    ImageView i2;
    if (!image_from_value(interp, b1, "EDGE", line, col, &i1) ||
        !image_from_value(interp, b2, "EDGE", line, col, &i2)) {
        value_free(b1);
        value_free(b2);
        return value_null();
    }
    ...
    for (size_t x = 0; x < i1.w; x++) {
        for (size_t y = 0; y < i1.h; y++) {
            ...
            dt->data[od + 3] = value_int(i1.t->data[o1 + 3].as.i);
        }
    }
...
static int parse_palette(Interpreter *interp, Value v, int **out_colors, size_t *out_n, int line, int col) {
    ...
    *out_n = 0;
    ...
    if (t->ndim == 1 && (t->shape[0] == 3 || t->shape[0] == 4)) {
        ...
        for (size_t c = 0; c < 4; c++) {
            cols[c] = (c < t->shape[0] && t->data[c].type == VAL_INT) ? clamp_u8_i64(t->data[c].as.i) : 255;
        }
        if (t->shape[0] == 3) {
            cols[3] = 255;
        }
...
static Value op_cellshade(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_range(interp, argc, 2, 2, "CELLSHADE", line, col)) {
        return value_null();
    }
    int *palette = NULL;
    size_t pal_n = 0;
    if (!parse_palette(interp, args[1], &palette, &pal_n, line, col)) {
        return value_null();
    }
