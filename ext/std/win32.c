...
// Operators
static Value op_win_message_box(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 1 || argc > 2) {
...
static Value op_win_last_error(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                               int col) {
    (void)interp;
    (void)args;
    (void)argc;
...
static Value op_win_load_library(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                 int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 1) {
    ...
    if (!h) {
        ...
        char em[128];
        snprintf(em, sizeof(em), "LoadLibrary failed: %lu", (unsigned long)err);
...
static Value op_win_free_library(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                 int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 1) {
    ...
    if (!ok) {
        DWORD err = GetLastError();
        char em[128];
        snprintf(em, sizeof(em), "FreeLibrary failed: %lu", (unsigned long)err);
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
...
static Value op_win_get_proc_address(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                     int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 2) {
    ...
    if (!p) {
        DWORD err = GetLastError();
        char em[128];
        snprintf(em, sizeof(em), "GetProcAddress failed: %lu", (unsigned long)err);
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
...
static Value op_win_create_file(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 5) {
    ...
    const char *path = value_as_cstr(interp, args[0], "path", line, col);
    if (!path) {
        return value_null();
    }
    long long access = value_as_int(interp, args[1], "access", line, col);
    if (interp->error) {
        return value_null();
    }
    long long share = value_as_int(interp, args[2], "share", line, col);
    if (interp->error) {
        return value_null();
    }
    long long creation = value_as_int(interp, args[3], "creation", line, col);
    if (interp->error) {
        return value_null();
    }
    long long flags = value_as_int(interp, args[4], "flags", line, col);
    if (interp->error) {
        return value_null();
    }
    ...
    if (h == INVALID_HANDLE_VALUE) {
        DWORD err = GetLastError();
        char em[128];
        snprintf(em, sizeof(em), "CreateFile failed: %lu", (unsigned long)err);
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
    return value_int((int64_t)(intptr_t)h);
...
static Value op_win_read_file(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 2) {
    ...
    long long handle = value_as_int(interp, args[0], "handle", line, col);
    if (interp->error) {
        return value_null();
    }
    long long length = value_as_int(interp, args[1], "length", line, col);
    if (interp->error) {
        return value_null();
    }
    if (length < 0) {
    ...
    if (!ok) {
        ...
        free(buf);
        char em[128];
        snprintf(em, sizeof(em), "ReadFile failed: %lu", (unsigned long)err);
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
    buf[read] = '\0';
...
static Value op_win_write_file(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                               int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 2) {
    ...
    if (!ok) {
        DWORD err = GetLastError();
        char em[128];
        snprintf(em, sizeof(em), "WriteFile failed: %lu", (unsigned long)err);
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
...
static Value op_win_close_handle(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                 int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 1) {
        set_runtime_error(interp, "WIN_CLOSE_HANDLE requires 1 arg (handle)", line, col);
        return value_null();
    }
    long long handle = value_as_int(interp, args[0], "handle", line, col);
    if (interp->error) {
    ...
    if (!ok) {
        DWORD err = GetLastError();
        char em[128];
        snprintf(em, sizeof(em), "CloseHandle failed: %lu", (unsigned long)err);
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
    return value_bool(ok != 0);
...
static Value op_win_virtual_alloc(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                  int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 3) {
        set_runtime_error(interp, "WIN_VIRTUAL_ALLOC requires 3 args", line, col);
        return value_null();
    }
    size_t size = (size_t)value_as_int(interp, args[0], "size", line, col);
    if (interp->error) {
        return value_null();
    }
    DWORD alloc_type = (DWORD)value_as_int(interp, args[1], "alloc_type", line, col);
    if (interp->error) {
        return value_null();
    }
    DWORD protect = (DWORD)value_as_int(interp, args[2], "protect", line, col);
    if (interp->error) {
        return value_null();
    }
    ...
    if (!p) {
        DWORD err = GetLastError();
        char em[128];
        snprintf(em, sizeof(em), "VirtualAlloc failed: %lu", (unsigned long)err);
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
    return value_int((int64_t)(intptr_t)p);
...
static Value op_win_virtual_free(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                 int col) {
    (void)arg_nodes;
    (void)env;
    if (argc < 3) {
        set_runtime_error(interp, "WIN_VIRTUAL_FREE requires 3 args", line, col);
        return value_null();
    }
    LPVOID addr = (LPVOID)(intptr_t)value_as_int(interp, args[0], "addr", line, col);
    if (interp->error) {
        return value_null();
    }
    SIZE_T size = (SIZE_T)value_as_int(interp, args[1], "size", line, col);
    if (interp->error) {
        return value_null();
    }
    DWORD free_type = (DWORD)value_as_int(interp, args[2], "free_type", line, col);
    if (interp->error) {
        return value_null();
    }
    ...
    if (!ok) {
        DWORD err = GetLastError();
        char em[128];
        snprintf(em, sizeof(em), "VirtualFree failed: %lu", (unsigned long)err);
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
    return value_bool(ok != 0);
...
static Value op_win_format_message(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                   int col) {
    (void)arg_nodes;
    (void)env;
    DWORD code = 0;
    if (argc >= 1) {
        code = (DWORD)value_as_int(interp, args[0], "code", line, col);
    }
    if (interp->error) {
        return value_null();
    }
    ...
    if (!len) {
        char em[128];
        snprintf(em, sizeof(em), "FormatMessage failed: %lu", (unsigned long)GetLastError());
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
...
static Value op_win_call(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    (void)env;
    if (argc < 4) {
        set_runtime_error(interp, "WIN_CALL requires at least 4 arguments", line, col);
        return value_null();
    }
    // args: lib, func, arg_types, ret_type, ...
    const char *lib = value_as_cstr(interp, args[0], "lib", line, col);
    if (!lib) {
        return value_null();
    }
    const char *func = value_as_cstr(interp, args[1], "func", line, col);
    if (!func) {
        return value_null();
    }
    ...
    char libbuf[1024];
    if (strlen(lib) >= sizeof(libbuf) - 5) {
        set_runtime_error(interp, "library name too long", line, col);
        return value_null();
    }
    if (prefix_stricmp(lib + (strlen(lib) >= 4 ? strlen(lib) - 4 : 0), ".dll") == 0) {
        strncpy(libbuf, lib, sizeof(libbuf) - 1);
        libbuf[sizeof(libbuf) - 1] = '\0';
    } else {
        {
            snprintf(libbuf, sizeof(libbuf), "%s.dll", lib);
        }
    }
    HMODULE h = LoadLibraryA(libbuf);
    if (!h) {
        char em[128];
        snprintf(em, sizeof(em), "Failed to load DLL %s: %lu", libbuf, (unsigned long)GetLastError());
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
    ...
    if (!p) {
        char em[128];
        snprintf(em, sizeof(em), "Function %s not found in %s: %lu", func, libbuf, (unsigned long)GetLastError());
        set_runtime_error(interp, em, line, col);
        return value_null();
    }
    ...
    int code_count = 0;
    char codes[64];
    strncpy(codes, arg_types, sizeof(codes) - 1);
    codes[sizeof(codes) - 1] = '\0';
    // if comma-separated, compact
    char compact[64];
    int cc = 0;
    for (int i = 0; codes[i] && cc + 1 < (int)sizeof(compact); i++) {
        if (codes[i] != ',') {
            compact[cc++] = codes[i];
        }
    }
    ...
    if (code_count > 6) {
        set_runtime_error(interp, "WIN_CALL supports up to 6 arguments", line, col);
        return value_null();
    }
    if (provided < code_count) {
        set_runtime_error(interp, "Not enough argument values for WIN_CALL", line, col);
        return value_null();
    }
    for (int i = 0; i < code_count; i++) {
        ...
        if (c == 'I' || c == 'P') {
            ...
            set_runtime_error(interp, "unsupported arg type code in WIN_CALL", line, col);
            return value_null();
        }
    }
    ...
    case 0:
        ...
        break;
    case 1:
        cres = ((fn1_t)p)(vargs[0]);
        break;
    case 2:
        cres = ((fn2_t)p)(vargs[0], vargs[1]);
        break;
    case 3:
        cres = ((fn3_t)p)(vargs[0], vargs[1], vargs[2]);
        break;
    case 4:
        cres = ((fn4_t)p)(vargs[0], vargs[1], vargs[2], vargs[3]);
        break;
    case 5:
        cres = ((fn5_t)p)(vargs[0], vargs[1], vargs[2], vargs[3], vargs[4]);
        break;
    case 6:
        cres = ((fn6_t)p)(vargs[0], vargs[1], vargs[2], vargs[3], vargs[4], vargs[5]);
        break;
    default:
        set_runtime_error(interp, "unsupported arg count", line, col);
        return value_null();
    }

    // Free any allocated wide strings used for string args
    for (int i = 0; i < code_count; i++) {
        if (compact[i] == 'S' && vargs[i]) {
            free((void *)vargs[i]);
        }
    }
...
void prefix_extension_init(prefix_ext_context *ctx) {
    ...
    ctx->register_operator("WIN_CALL", (prefix_operator_fn)op_win_call,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_MESSAGE_BOX", (prefix_operator_fn)op_win_message_box,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_SLEEP", (prefix_operator_fn)op_win_sleep,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_LAST_ERROR", (prefix_operator_fn)op_win_last_error,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_LOAD_LIBRARY", (prefix_operator_fn)op_win_load_library,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_FREE_LIBRARY", (prefix_operator_fn)op_win_free_library,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_GET_PROC_ADDRESS", (prefix_operator_fn)op_win_get_proc_address,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_CREATE_FILE", (prefix_operator_fn)op_win_create_file,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_READ_FILE", (prefix_operator_fn)op_win_read_file,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_WRITE_FILE", (prefix_operator_fn)op_win_write_file,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_CLOSE_HANDLE", (prefix_operator_fn)op_win_close_handle,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_VIRTUAL_ALLOC", (prefix_operator_fn)op_win_virtual_alloc,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("WIN_VIRTUAL_FREE", (prefix_operator_fn)op_win_virtual_free,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
