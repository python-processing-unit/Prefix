...
static int add_handle(NetHandle **arr, size_t *count, size_t *cap, SOCKET sock) {
    if (reserve_handles(arr, cap, *count + 1) != 0) {
        return -1;
    }
...
static char *normalize_encoding_name(const char *coding) {
    if (!coding || coding[0] == '\0') {
        return strdup("UTF-8");
    }
    if (prefix_stricmp(coding, "UTF8") == 0 || prefix_stricmp(coding, "UTF-8") == 0) {
        return strdup("UTF-8");
    }
    if (prefix_stricmp(coding, "UTF16") == 0 || prefix_stricmp(coding, "UTF-16") == 0) {
        return strdup("UTF-16");
    }
    if (prefix_stricmp(coding, "ASCII") == 0) {
        return strdup("ASCII");
    }
    if (prefix_stricmp(coding, "LATIN1") == 0 || prefix_stricmp(coding, "LATIN-1") == 0) {
        return strdup("latin-1");
    }
    if (prefix_stricmp(coding, "ANSI") == 0) {
...
static const char *py_bridge_script(void) {
    return "import sys,ssl,io,urllib.request,ftplib,smtplib\n"
           ...
           "    if tls:\n"
           "      ftp=ftplib.FTP_TLS(); ftp.context=(ssl.create_default_context() if verify else "
           ...
           "    msg='\\r\\n'.join([f'From: {mfrom}','To: ' + ', '.join(rec),f'Subject: {subj}','MIME-Version: "
           "1.0','Content-Type: text/plain; charset=utf-8','',''+body,''])\n"
           "    if tls:\n"
...
static int ensure_bridge_script(char *out_path, size_t out_path_cap) {
    ...
    if (snprintf(script_path, sizeof(script_path), "%s%s", temp_dir, "prefix_networking_bridge.py") < 0) {
        return -1;
    }
    const char *code = py_bridge_script();
    if (write_text_file(script_path, code) != 0) {
        return -1;
    }
...
static int create_temp_file_path(char *out_path, size_t out_cap) {
    ...
    if (GetTempFileNameA(temp_dir, "pfx", 0, temp_file) == 0) {
        return -1;
    }
...
static int bridge_call(const char *op, const char **args, int arg_count, unsigned char **out_data, size_t *out_len,
    ...
    *out_err = strdup("bridge unsupported on this platform");
    ...
    if (create_temp_file_path(req_path, sizeof(req_path)) != 0 ||
        create_temp_file_path(rsp_path, sizeof(rsp_path)) != 0) {
...
static int winhttp_request(const char *method_u8, const char *url_u8, const unsigned char *body, size_t body_len,
                           const char *content_type_u8, int timeout_ms, int verify, int *out_status,
                           unsigned char **out_body, size_t *out_body_len) {
    int ret = -1;
    ...
    hSession = WinHttpOpen(L"Prefix-C/networking", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME,
    ...
    hRequest =
        WinHttpOpenRequest(hConnect, wmethod, path, NULL, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, req_flags);
    if (!hRequest) {
        goto cleanup;
    }

    if (!verify && uc.nScheme == INTERNET_SCHEME_HTTPS) {
        DWORD sec = SECURITY_FLAG_IGNORE_CERT_CN_INVALID | SECURITY_FLAG_IGNORE_CERT_DATE_INVALID |
    ...
    if (!WinHttpSendRequest(hRequest, headers_ptr, headers_len, (LPVOID)body, (DWORD)body_len, (DWORD)body_len, 0)) {
        free(content_type_w);
        goto cleanup;
    }
    free(content_type_w);
...
static Value op_tcp_connect(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    (void)verify;
    if (argc >= 3) {
        EXPECT_INT_AT(args, 2, "TCP_CONNECT", interp, line, col);
    }
    if (argc >= 4) {
        EXPECT_INT_AT(args, 3, "TCP_CONNECT", interp, line, col);
    }
    if (argc >= 5) {
        EXPECT_INT_AT(args, 4, "TCP_CONNECT", interp, line, col);
    }
    if (argc >= 6) {
        EXPECT_STR_AT(args, 5, "TCP_CONNECT", interp, line, col);
    }

    if (port < 0 || port > 65535) {
        RUNTIME_ERROR(interp, "TCP_CONNECT: port out of range", line, col);
    }
    if (tls != 0) {
        RUNTIME_ERROR(interp, "TCP_CONNECT: TLS not supported in C extension build", line, col);
    }
...
static Value op_tcp_send(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (argc >= 3) {
        EXPECT_STR_AT(args, 2, "TCP_SEND", interp, line, col);
    }
    ...
    if (s == INVALID_SOCKET) {
        RUNTIME_ERROR(interp, "TCP_SEND: invalid handle", line, col);
    }

    const char *coding = (argc >= 3) ? as_cstr(args[2]) : "UTF-8";
    char *norm = normalize_encoding_name(coding);
    if (!norm) {
        RUNTIME_ERROR(interp, "TCP_SEND failed: out of memory", line, col);
    }
    if (prefix_stricmp(norm, "UTF-8") != 0 && prefix_stricmp(norm, "ASCII") != 0 &&
        prefix_stricmp(norm, "latin-1") != 0 && prefix_stricmp(norm, "cp1252") != 0) {
        free(norm);
    ...
    if (n == SOCKET_ERROR) {
        RUNTIME_ERROR(interp, "TCP_SEND failed", line, col);
    }
    return value_int(n);
}

static Value op_tcp_recv_text(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 3) {
        EXPECT_STR_AT(args, 2, "TCP_RECV_TEXT", interp, line, col);
    }

    int hid = (int)as_i64(args[0]);
    int64_t max_bytes = as_i64(args[1]);
    if (max_bytes <= 0 || max_bytes > 16 * 1024 * 1024) {
        RUNTIME_ERROR(interp, "TCP_RECV_TEXT: max_bytes must be > 0", line, col);
    }
    SOCKET s = find_handle(g_state.tcp, g_state.tcp_count, hid);
    if (s == INVALID_SOCKET) {
        RUNTIME_ERROR(interp, "TCP_RECV_TEXT: invalid handle", line, col);
    }

    char *buf = (char *)malloc((size_t)max_bytes + 1);
    if (!buf) {
        RUNTIME_ERROR(interp, "TCP_RECV_TEXT failed: out of memory", line, col);
    }
...
static Value op_tcp_recv_bytes(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                               int col) {
    (void)arg_nodes;
    (void)env;
    ...
    int64_t max_bytes = as_i64(args[1]);
    if (max_bytes <= 0 || max_bytes > 16 * 1024 * 1024) {
        RUNTIME_ERROR(interp, "TCP_RECV_BYTES: max_bytes must be > 0", line, col);
    }
    SOCKET s = find_handle(g_state.tcp, g_state.tcp_count, hid);
    if (s == INVALID_SOCKET) {
        RUNTIME_ERROR(interp, "TCP_RECV_BYTES: invalid handle", line, col);
    }

    unsigned char *buf = (unsigned char *)malloc((size_t)max_bytes);
    if (!buf) {
        RUNTIME_ERROR(interp, "TCP_RECV_BYTES failed: out of memory", line, col);
    }
...
static Value op_udp_bind(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (argc >= 3) {
        EXPECT_INT_AT(args, 2, "UDP_BIND", interp, line, col);
    }
    ensure_socket_runtime();
    ...
    if (port < 0 || port > 65535) {
        RUNTIME_ERROR(interp, "UDP_BIND: port out of range", line, col);
    }
...
static Value op_udp_send(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (argc >= 5) {
        EXPECT_STR_AT(args, 4, "UDP_SEND", interp, line, col);
    }
    ...
    if (port < 0 || port > 65535) {
        RUNTIME_ERROR(interp, "UDP_SEND: port out of range", line, col);
    }

    SOCKET s = find_handle(g_state.udp, g_state.udp_count, hid);
    if (s == INVALID_SOCKET) {
        RUNTIME_ERROR(interp, "UDP_SEND: invalid handle", line, col);
    }
    ...
    if (n == SOCKET_ERROR) {
        RUNTIME_ERROR(interp, "UDP_SEND failed", line, col);
    }
    return value_int(n);
}

static Value op_udp_recv_text(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 3) {
        EXPECT_INT_AT(args, 2, "UDP_RECV_TEXT", interp, line, col);
    }
    if (argc >= 4) {
        EXPECT_STR_AT(args, 3, "UDP_RECV_TEXT", interp, line, col);
    }
    ...
    int64_t timeout_ms = (argc >= 3) ? as_i64(args[2]) : 0;
    if (max_bytes <= 0 || max_bytes > 16 * 1024 * 1024) {
        RUNTIME_ERROR(interp, "UDP_RECV_TEXT: max_bytes must be > 0", line, col);
    }

    SOCKET s = find_handle(g_state.udp, g_state.udp_count, hid);
    if (s == INVALID_SOCKET) {
        RUNTIME_ERROR(interp, "UDP_RECV_TEXT: invalid handle", line, col);
    }
    set_socket_timeout_ms(s, ms_to_timeout_ms(timeout_ms));

    char *buf = (char *)malloc((size_t)max_bytes + 1);
    if (!buf) {
        RUNTIME_ERROR(interp, "UDP_RECV_TEXT failed: out of memory", line, col);
    }
...
static Value op_udp_recv_bytes(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                               int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 3) {
        EXPECT_INT_AT(args, 2, "UDP_RECV_BYTES", interp, line, col);
    }
    ...
    int64_t timeout_ms = (argc >= 3) ? as_i64(args[2]) : 0;
    if (max_bytes <= 0 || max_bytes > 16 * 1024 * 1024) {
        RUNTIME_ERROR(interp, "UDP_RECV_BYTES: max_bytes must be > 0", line, col);
    }

    SOCKET s = find_handle(g_state.udp, g_state.udp_count, hid);
    if (s == INVALID_SOCKET) {
        RUNTIME_ERROR(interp, "UDP_RECV_BYTES: invalid handle", line, col);
    }
    set_socket_timeout_ms(s, ms_to_timeout_ms(timeout_ms));

    unsigned char *buf = (unsigned char *)malloc((size_t)max_bytes);
    if (!buf) {
        RUNTIME_ERROR(interp, "UDP_RECV_BYTES failed: out of memory", line, col);
    }
    struct sockaddr_in from;
#ifdef _WIN32
    int from_len = (int)sizeof(from);
...
static Value op_http_get_text(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 2) {
        EXPECT_INT_AT(args, 1, "HTTP_GET_TEXT", interp, line, col);
    }
    if (argc >= 3) {
        EXPECT_INT_AT(args, 2, "HTTP_GET_TEXT", interp, line, col);
    }
...
static Value op_http_get_bytes(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                               int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 2) {
        EXPECT_INT_AT(args, 1, "HTTP_GET_BYTES", interp, line, col);
    }
    if (argc >= 3) {
        EXPECT_INT_AT(args, 2, "HTTP_GET_BYTES", interp, line, col);
    }
...
static Value op_http_get_status(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 2) {
        EXPECT_INT_AT(args, 1, "HTTP_GET_STATUS", interp, line, col);
    }
    if (argc >= 3) {
        EXPECT_INT_AT(args, 2, "HTTP_GET_STATUS", interp, line, col);
    }
...
static Value op_http_post_text(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                               int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 3) {
        EXPECT_STR_AT(args, 2, "HTTP_POST_TEXT", interp, line, col);
    }
    if (argc >= 4) {
        EXPECT_INT_AT(args, 3, "HTTP_POST_TEXT", interp, line, col);
    }
    if (argc >= 5) {
        EXPECT_INT_AT(args, 4, "HTTP_POST_TEXT", interp, line, col);
    }
    ...
    size_t rlen = 0;
    if (winhttp_request("POST", url, (const unsigned char *)body_txt, strlen(body_txt), content_type, timeout_ms,
                        verify != 0, &status, &resp, &rlen) != 0) {
...
static Value op_ftp_list(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (argc >= 6) {
        EXPECT_INT_AT(args, 5, "FTP_LIST", interp, line, col);
    }
    if (argc >= 7) {
        EXPECT_INT_AT(args, 6, "FTP_LIST", interp, line, col);
    }
    if (argc >= 8) {
        EXPECT_INT_AT(args, 7, "FTP_LIST", interp, line, col);
    }

    int64_t port = as_i64(args[1]);
    if (port < 0 || port > 65535) {
        RUNTIME_ERROR(interp, "FTP_LIST: port out of range", line, col);
    }
    ...
    const char *bargs[8] = {as_cstr(args[0]), port_buf, as_cstr(args[2]), as_cstr(args[3]),
                            as_cstr(args[4]), tls_buf,  timeout_buf,      verify_buf};
...
static Value op_ftp_get_bytes(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 6) {
        EXPECT_INT_AT(args, 5, "FTP_GET_BYTES", interp, line, col);
    }
    if (argc >= 7) {
        EXPECT_INT_AT(args, 6, "FTP_GET_BYTES", interp, line, col);
    }
    if (argc >= 8) {
        EXPECT_INT_AT(args, 7, "FTP_GET_BYTES", interp, line, col);
    }

    int64_t port = as_i64(args[1]);
    if (port < 0 || port > 65535) {
        RUNTIME_ERROR(interp, "FTP_GET_BYTES: port out of range", line, col);
    }
...
static Value op_ftp_put_bytes(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                              int col) {
    (void)arg_nodes;
    (void)env;
    ...
    if (argc >= 7) {
        EXPECT_INT_AT(args, 6, "FTP_PUT_BYTES", interp, line, col);
    }
    if (argc >= 8) {
        EXPECT_INT_AT(args, 7, "FTP_PUT_BYTES", interp, line, col);
    }
    if (argc >= 9) {
        EXPECT_INT_AT(args, 8, "FTP_PUT_BYTES", interp, line, col);
    }

    int64_t port = as_i64(args[1]);
    if (port < 0 || port > 65535) {
        RUNTIME_ERROR(interp, "FTP_PUT_BYTES: port out of range", line, col);
    }
    ...
    free(payload);
    if (!payload_hex) {
        RUNTIME_ERROR(interp, "FTP_PUT_BYTES failed: out of memory", line, col);
    }
    ...
    const char *bargs[9] = {as_cstr(args[0]), port_buf, as_cstr(args[2]), as_cstr(args[3]), as_cstr(args[4]),
                            payload_hex,      tls_buf,  timeout_buf,      verify_buf};
    unsigned char *out = NULL;
    size_t out_len = 0;
    char *err = NULL;
...
static Value op_smtp_send(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line, int col) {
    ...
    if (argc >= 9) {
        EXPECT_INT_AT(args, 8, "SMTP_SEND", interp, line, col);
    }
    if (argc >= 10) {
        EXPECT_INT_AT(args, 9, "SMTP_SEND", interp, line, col);
    }
    if (argc >= 11) {
        EXPECT_INT_AT(args, 10, "SMTP_SEND", interp, line, col);
    }

    int64_t port = as_i64(args[1]);
    if (port < 0 || port > 65535) {
        RUNTIME_ERROR(interp, "SMTP_SEND: port out of range", line, col);
    }
    ...
    const char *bargs[11] = {as_cstr(args[0]), port_buf,         as_cstr(args[2]), as_cstr(args[3]),
                             as_cstr(args[4]), as_cstr(args[5]), as_cstr(args[6]), as_cstr(args[7]),
                             tls_buf,          timeout_buf,      verify_buf};
    unsigned char *out = NULL;
    size_t out_len = 0;
    char *err = NULL;
...
void prefix_extension_init(prefix_ext_context *ctx) {
    ...
    ensure_socket_runtime();
    ctx->register_operator("TCP_CONNECT", op_tcp_connect,
    ...
    ctx->register_operator("TCP_RECV_TEXT", op_tcp_recv_text,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("TCP_RECV_BYTES", op_tcp_recv_bytes,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ...
    ctx->register_operator("UDP_RECV_TEXT", op_udp_recv_text,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("UDP_RECV_BYTES", op_udp_recv_bytes,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("UDP_CLOSE", op_udp_close, PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);

    ctx->register_operator("HTTP_GET_TEXT", op_http_get_text,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("HTTP_GET_BYTES", op_http_get_bytes,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("HTTP_GET_STATUS", op_http_get_status,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("HTTP_POST_TEXT", op_http_post_text,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);

    ctx->register_operator("FTP_LIST", op_ftp_list, PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("FTP_GET_BYTES", op_ftp_get_bytes,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
    ctx->register_operator("FTP_PUT_BYTES", op_ftp_put_bytes,
                           PREFIX_EXTENSION_ASMODULE | PREFIX_EXTENSION_MODULE_RESTRICTED);
