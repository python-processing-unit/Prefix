...
static Value op_tst_ext_package_submod(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                       int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_EXT_PACKAGE_SUBMOD", line, col)) {
        return value_null();
    }
