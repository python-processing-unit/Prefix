...
static Value op_tst_ext_package_submod(Interpreter* interp, Value* args, int argc, Expr** arg_nodes, Env* env, int line, int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_EXT_PACKAGE_SUBMOD", line, col)) return value_null();
    return value_str("package-submod");
}
...
void prefix_extension_init(prefix_ext_context* ctx) {
    if (!ctx) return;
    ctx->register_operator("TST_EXT_PACKAGE_SUBMOD", op_tst_ext_package_submod, 0);
}