...
static void hook_log_event(Interpreter *interp, const char *event_name) {
    (void)interp;
    if (!g_event_log_path || !event_name || event_name[0] == '\0') {
        return;
    }
...
static Value op_tst_hooks_ready(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_HOOKS_READY", line, col)) {
        return value_null();
    }
...
void prefix_extension_init(prefix_ext_context *ctx) {
    ...
    if (!g_event_log_path) {
        g_event_log_path = dup_env("PREFIX_TEST_EVENT_LOG");
    }
    if (!g_periodic_log_path) {
        g_periodic_log_path = dup_env("PREFIX_TEST_PERIODIC_LOG");
    }
    if (!g_repl_log_path) {
        g_repl_log_path = dup_env("PREFIX_TEST_REPL_LOG");
    }
