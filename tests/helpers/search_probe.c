...
static char* capture_library_path(void) {
    ...
    if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                            (LPCSTR)(const void*)&prefix_extension_init,
                            &module)) {
        return NULL;
    }
...
static Value op_tst_search_probe_path(Interpreter* interp, Value* args, int argc, Expr** arg_nodes, Env* env, int line, int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_SEARCH_PROBE_PATH", line, col)) return value_null();
