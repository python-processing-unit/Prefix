...
static Value op_tst_ext_global(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                               int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_EXT_GLOBAL", line, col)) {
        return value_null();
    }
    return value_str("core-global");
}

static Value op_tst_ext_init_count(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                   int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_EXT_INIT_COUNT", line, col)) {
        return value_null();
    }
    return value_int(g_init_count);
}

static Value op_tst_ext_api_version(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                    int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_EXT_API_VERSION", line, col)) {
        return value_null();
    }
    return value_int(g_api_version);
}

static Value op_tst_ext_restricted(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                   int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_EXT_RESTRICTED", line, col)) {
        return value_null();
    }
    return value_str("core-restricted");
}

static Value op_tst_ext_asmodule_only(Interpreter *interp, Value *args, int argc, Expr **arg_nodes, Env *env, int line,
                                      int col) {
    (void)args;
    (void)arg_nodes;
    (void)env;
    if (!expect_argc_eq(interp, argc, 0, "TST_EXT_ASMODULE_ONLY", line, col)) {
        return value_null();
    }
...
void prefix_extension_init(prefix_ext_context *ctx) {
    ...
    ctx->register_operator("TST_EXT_ASMODULE_ONLY", op_tst_ext_asmodule_only, PREFIX_EXTENSION_ASMODULE);
}